...
| **Browser sessions are per-subagent** | Each testing subagent runs `agent-browser open <url>` and `agent-browser close` within its own lifetime. Review subagents don't need browsers. |
...
- [ ] Each subagent uses independent browser session (`agent-browser open <url>`)
